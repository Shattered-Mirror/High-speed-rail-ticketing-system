/* booking.c（增加保存/加载，与 train 的 seatmark 协同恢复） */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "booking.h"
...
int booking_create(BookingList *BL, TrainList *TL, PassengerList *PL,
    ...
    } else { b.depart_time[0] = '\0'; b.price = 0.0; }
    ...
    BL->data[BL->size++] = b;
    rebuild(BL);
    if (out_order_id) { strncpy(out_order_id, b.order_id, order_len-1); out_order_id[order_len-1] = 0; }
    return 0;
}
...
/* -------------------- 持久化：保存 / 载入 bookings（与 train 协同恢复 seatmap） -------------------- */
...
/* load_bookings: 读取 booking 并把记录放入 BL；对于未退票记录（canceled==0），在 Train 上标记对应占用（train_mark_seat） */
int load_bookings(const char *filename, BookingList *L, TrainList *TL) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        char *parts[16]; int p = 0;
        char *tok = strtok(line, "|");
        while (tok && p < 16) { parts[p++] = tok; tok = strtok(NULL, "|"); }
        if (p < 15) { fclose(f); return 0; }
        Booking b;
        memset(&b, 0, sizeof(b));
        ...
        /* append to booking list without allocation (seatmaps will be restored below) */
        if (L->size >= L->capacity) bookinglist_expand(L);
        L->data[L->size++] = b;

        /* if not canceled, mark seat on train */
        if (!b.canceled) {

...
static int input_int(const char *prompt) {
    char buf[64]; input_line(prompt, buf, sizeof(buf)); return atoi(buf);
}
        ...
        if (ch == 0) {
            ...
            for (;;) {
                ...
                if (c == 1) {
                    ...
                    char id[ID_LEN]; input_line("输入车次号: ", id, sizeof(id));
                    int idx = train_find_index(&TL, id);
                    if (idx == -1) puts("未找到");
                    else {
            ...
            for (;;) {
                ...
                if (c == 1) {
                    ...
                    char id[ID_LEN]; input_line("证件号: ", id, sizeof(id));
                    if (passenger_delete(&PL, id) == 0) puts("删除成功"); else puts("未找到");
                } else if (c == 3) {
                    char id[ID_LEN]; input_line("证件号: ", id, sizeof(id));
            ...
            for (;;) {
                ...
                if (c == 1) {
                    ...
                    if (res == 0) printf("订票成功，订单号: %s\n", orderid);
                    else if (res == -1) puts("乘客不存在");
                    else if (res == -2) puts("无余票/分配失败");
                    else puts("订票失败");
                } else if (c == 2) {
                    char oid[ORDER_ID_LEN]; input_line("输入订单号退票: ", oid, sizeof(oid));
                    int res = booking_cancel(&BL, oid, &TL);
                    if (res == 0) puts("退票成功");
                    else if (res == -1) puts("未找到订单");
                    else if (res == -2) puts("订单已退票");
                    else puts("退票异常");
                } else if (c == 3) {
                    char oid[ORDER_ID_LEN]; input_line("订单号: ", oid, sizeof(oid));
                    ...
                    char name[NAME_LEN]; input_line("姓名: ", name, sizeof(name));
                    int found = 0;
                    for (int i = 0; i < BL.size; ++i) if (strcmp(BL.data[i].passenger_name, name) == 0) {
                    ...
                    input_line("日期(YYYY-MM-DD): ", date, sizeof(date));
                    int found = 0;
                    for (int i = 0; i < BL.size; ++i) if (strcmp(BL.data[i].train_id, train_id) == 0 && strcmp(BL.data[i].date, date) == 0) {
                        printf("订单:%s  %s->%s  %s\n", BL.data[i].order_id, BL.data[i].from, BL.data[i].to, BL.data[i].seat_no);
                        found++;
                    }
                    if (!found) puts("未找到");
